...
class PhoneBook{	// 내가 만드는 자료형
	...
	// 2. 기능 (member method) - 객체에 대한 함수
	// 매서드 내부에서는 멤버 필드를 조건없이 자유롭게 참조할 수 있다
	void show() {
		String form = "%s (%d) : %s\n";
		System.out.printf(form, name, age, phoneNumber);
	}
	
	// 3. 생성자 : 객체를 생성할 때 실행되는 함수이며, 클래스의 이름과 같은 이름을 사용하는 함수
