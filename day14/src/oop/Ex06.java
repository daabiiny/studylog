...
class Test{
	...
	// 생성자를 작성하지 않아도 컴파일시 기본생성자(매개변수를 받지않는 생성자)가 자동으로 작성된다
	// Test() {}
	
	// 작성자가 임으로 생성자를 정의하면, 자동으로 생성자를 만들어주지 않는다
	Test(int num) {
...
/*		
 		...
 		4) 생성자는 매개변수를 받아서, 멤버필드의 값을 초기화 할 수 있다
 		-> 클래스는 하나지만, 생성자코드에 따라 서로 다른 객체가 서로 다른 값을 가질 수 있도록 한다
 		
 		5) 생성자는 필드 초기화 이외에도, 특정 코드를 수행하도록 만들 수 있다
 		
 */
		// Scanner클래스는 객체를 생성하기 위해서, InputSteam 혹은 String 혹은 File등을 전달해야 한다
