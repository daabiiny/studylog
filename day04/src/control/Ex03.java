...
public class Ex03 {
	public static void main(String[] args) {
		...
		// else는 단독으로 사용할 수 없고, 이전에 if가 나타나 있어야하고 연속되어있어야한다
...
//		System.out.println("다른 코드를 작성하면 연속성이 무너지므로, else를 실행할 수 없다");
		...
		// if문은 true일때만 실행(false는 아무것도 실행안됨)
		if (age >= 20) {
			if (gender % 2 != 0) {
		...
		int num = 5;
		// ctrl + shift + f
		// 중첩된 제어문을 사용할 때는 들여쓰기 시작점을 맞춰야 한다
		...
		else {								// (num이 1, 2, 3도 아닐때)
			System.out.println("wrong");
		}
		// if ~ else if ~ else
		// 여러 선택지 중 반드시 하나를 골라서 실행해야할 경우
