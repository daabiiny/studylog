...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 순서가 있으며(index를 쓴다는 것), 값의 중복을 허용한다
		
		// java.uil.list는 인터페이스라서 객체를 생성할 수 없다
		...
		// 리스트 및 컬렉션은 대부분 toString()이 오버라이딩되어있어서 바로 출력해도 내용을 확인할 수 있다
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
		// list.add(int index, E e) 함수는 지정한 인덱스에 값을 추가한다
		list.add(1, 30);	// add 함수는 오버로딩됐다고 할 수 있다!!
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
...
//		list.remove(30);					// 정수를 그냥 전달하면 index로 간주한다
		...
		list.sort(null);			// null 자리에는 정렬 기준을 나타내기 위한 객체를 전달할 수 있다 ( 반드시 null을 넣어야함 !)
		System.out.println(list);
