...
public class Ex01 {
	public static void main(String[] args) {
		...
		// System.out.println() : 괄호안에 하나의 값을 받아서 출력한다(*콤마를 넣을 수 없다) 출력후 한줄 내린다.
		...
		System.out.println("\r");	// \r은 커서를 줄 왼쪽끝으로 이동시킨다 (CarrigeReturn)
		...
		// lyrics에 처음 값을 지정한 후
		// 기존 값을 유지하면서 새로운 내용을 덧분이기(append)할 때 사용할 수 있다
		...
		System.out.println("1" + "2");	// 12	양쪽이 문자열, 글자모양을 이어붙인다 (문자열은 계산을 수행하지 않음, 문자열은 *안됨)
		
		// print와 println의 차이는 주로 반복문 내부에서 값을 연속으로 출력할 때 활용한다
		
		for(int i = 0; i < 5; i++) {
			System.out.print(i + " ");	// i를 출력하고 한칸 띄어쓰기하고 줄은 바꾸지 않기 
		}
		System.out.println();			// 출력이 다 끝나면 줄 바꾸기
		
		for(int i = 0; i < 5; i++) {
			System.out.println(i);		// 값을 출력할 때마다 계속 줄 바꾸기
		}
